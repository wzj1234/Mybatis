...
public class Calculate extends ActionSupport{	
    ...
    public List<GraphOne> getGraphOneListFunc(List<String> radioList) throws Exception {
    	...
    	List<Radio> graphOneHeads = saveEDao.queryRadioByRadios(radioList);
    	List<GraphOneModel> graphOneModels = saveEDao.queryLKJByRadios(radioList);
        session.close();
    ...
    public List<GraphTwo> getGraphTwoListFunc(List<String> radioList,List<String> ageList) throws Exception {
    	...
    	SqlSession session = Constants.getSqlSession();
    	SaveEDao saveEDao = session.getMapper(SaveEDao.class);
    	List<SVTRAModel> SVTRAModels = saveEDao.querySVTRAByRadios(radioList);
    	List<SVTModel> SVTModels = saveEDao.queryGraphTwoByRadios(radioList);
        session.close();
	...
	public void getAllOutputFile() throws Exception{
            ...
            List<SaveE> raeList = saveEDao.queryRAEList();
            List<SVTModel> SVTList = saveEDao.querySVTList();
            List<SVTRAModel> SVTRAList = saveEDao.querySVTRAList();
            List<GraphOneModel> GraphOneList = saveEDao.queryLKJList();
            session.close();
	...
	public void getAskOutputFile(List<String> radios,List<String> ages) throws Exception{
		...
		SqlSession session = Constants.getSqlSession();
		SaveEDao saveEDao = session.getMapper(SaveEDao.class);
        List<Radio> radioList = saveEDao.queryRadioByRadios(radios);
        List<SaveE> raeList = saveEDao.queryRAEByRadios(radios);
        List<SVTModel> SVTList = saveEDao.querySVTByRadios(radios);
        List<SVTRAModel> SVTRAList = saveEDao.querySVTRAByRadios(radios);
        List<GraphOneModel> GraphOneList = saveEDao.queryLKJByRadios(radios);
        session.close();
...
//          response.addHeader("Content-Disposition","attachment;filename=ChooseCalResult.xls");
	...
	public void downloadConclusion() throws Exception{
		...
		SqlSession session = Constants.getSqlSession();
		SaveEDao saveEDao = session.getMapper(SaveEDao.class);
        List<Radio> radioClassList = saveEDao.queryRadioByRadios(radioList);  
        List<SVTRAModel> SVTRAList = saveEDao.querySVTRAByRadios(radioList);
        session.close();
